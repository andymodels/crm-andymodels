const {
  ...
  isValidEmail,
  isValidPhoneBR,
} = require('./brValidators');

const TIPOS_CHAVE_PIX = ['CPF', 'CNPJ', 'E-mail', 'Celular', 'Aleatória'];

/**
 * Valida e normaliza chave Pix conforme tipo (padrão Bacen).
 * @returns {{ ok: true, value: string } | { ok: false, message: string }}
 */
function validateChavePixValue(tipoChave, raw) {
...
/**
 * Conta bancária (BR): banco, agência e conta com dígitos; tipo corrente/poupança.
 */
function validateContaBancariaBr(item, indexLabel) {
...
/**
 * Aceita formato legado { tipo, tipo_chave_pix, valor } e normaliza para estrutura nova.
 */
function sanitizeAndValidateFormasPagamentoArray(formas) {
...
module.exports = {
  TIPOS_CHAVE_PIX,
  sanitizeAndValidateFormasPagamentoArray,
  validateChavePixValue,
  validateContaBancariaBr,
};
